...
const RedisUniswapUtils = require('@ioc:App/Common/RedisUniswapUtils')
const HelperUtils = require('@ioc:App/Common/HelperUtils')

const UniswapModel = require('@ioc:App/Models/Uniswap')
...
export default class FetchUniswapEventJob implements JobContract {
  public key = 'FetchUniswapEventJob'

  public async handle(job) {
    const { data } = job
    console.log('uniswappp')

    const eventType = data.event_type
    let from = data.from
    let to = data.to

    if (await RedisUniswapUtils.existRedisUniswapBlockNumber(eventType)) {
      let redisData = await RedisUniswapUtils.getRedisUniswapBlockNumber(eventType)
    ...
    try {
      ...
      await RedisUniswapUtils.setRedisUniswapBlockNumber({ current: to, event_type: eventType })
    } catch (e) {
      Logger.error(e)
    }
  }
  ...
  private async fetchEvents(provider, event_type, from, to) {
    try {
      const instance = await HelperUtils.getUniswapContractInstance()
      ...
      for (const event of events) {
        ...
        data.event_info = JSON.stringify(event.returnValues)
